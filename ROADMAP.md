...
| M10 — Settings + polish | ✓ done (2026-05-28) | `SettingsSheet` (sheet-over-MacShell, 520×420) wired to ⌘, via `CommandGroup(replacing: .appSettings)` · base-currency `Picker` over curated 8 (USD/EUR/GBP/JPY/CHF/CAD/AUD/BRL) — change triggers `coordinator.refreshAll()` so new FX pairs land immediately · theme override (System/Light/Dark) persisted on `AppSettings.themeOverride` (new field) · `FolioThemeProvider` converted from ViewModifier to `View` wrapper so it `@Query`s settings, applies both `\.theme` env + `.preferredColorScheme(...)` so macOS chrome flips alongside theme tokens · "Reveal data folder…" via `NSWorkspace.activateFileViewerSelecting` on `container.configurations.first?.url` · `FXLookup.fxAt(rates:)` extracted from `HistoricalLookups`; wired into Stocks/Crypto/Overview's spot-quote reducers — non-USD base currency now actually works · `ToolbarSearchField` (real `TextField` + popover-anchored asset matches) replaces `ToolbarSearchStub`; `AssetSearch` `@Observable` model does case-insensitive substring on symbol+name; ⌘K wired via `CommandGroup(after: .pasteboard)` flipping `router.searchFocused` (round-tripped through `@FocusState`); Enter routes to Stocks/Crypto per asset kind · `EmptyState` primitive (icon + headline + sub + CTA) wired into all 4 MVP screens (Overview/Stocks/Crypto/Transactions) with "Add a transaction" CTA · 6 M10-tagged audit follow-ups closed: ⌘N→⌘+ for Add holding · timer pause/resume via `NSApplication.didResignActive`/`didBecomeActive` observers in `NotificationObserverBag` (heap class deinit-cleans observers without forcing main-actor isolation) · save-error filter classifies `NSCocoaErrorDomain` constraint codes as recoverable, surfaces others as `.failed` · `print` → `os_log` via new `FolioLog` namespace (subsystems: holdings/quotes/history/persist) · `MockQuoteProvider` `Decimal(string: "x")!` force-unwraps replaced with `Decimal(n) / Decimal(d)` · `Money` doc-comment explains mixed-currency `precondition` trap behavior · `Formatters.swift` (dead `FolioFormat`) deleted; inlined as `private enum PreviewFormat` into `DesignSystemPreview.swift` · 17 new tests (`FXLookupTests` × 5, `AssetSearchTests` × 7, `SettingsTests` × 5) · 148 tests green (131 → 148) |
| M11 — Test + verify | ✓ done (2026-05-28) | Closes MVP. Coverage gap-fill on the 4 milestone-named units: `PortfolioMetricsTests` (+5: 4 `ytdPerformance` branches — empty / all-pre-Jan1 / Jan-1-anchored / Jan-1-zero — plus `totalValue` all-nil-prices = zero) · `HoldingsReducerTests` (+3: exact-qty sell clamps to nil-output, multi-cycle buy/sell preserves weighted-avg cost, multi-currency buys use the FX rate at each txn date independently) · `MockQuoteProviderTests` (+3 new + 1 replaced: multi-symbol LCG state isolation, `FXProviderError.unsupportedPair` propagation for `CHF→JPY`, flat-rate historical FX over `.m1`; old `testFXRateUSDEURRoundtripCloseToOne` constants-tautology replaced with `testFXProviderRoundtripConsistency` that loops USD/EUR + USD/GBP through the provider's public API) · `MoneyTests` (+2: unknown-currency code `XXX` doesn't crash the `NumberFormatter` fallback, `(a+b)+c == a+(b+c)` associativity guards future precision-drift refactors) · `QuoteRefreshCoordinatorTests` (+1: `collectFXPairs` deduplicates duplicate EUR txns to a single (EUR, USD) pair and never emits USD→USD; required bumping `collectFXPairs` + `FXPair` from `private` → `internal`) · `SeedData.syntheticBuyDate` now carries a doc-comment marking it as the chronological floor for `seedMockTransactions` (M11 audit follow-up B4). Scope decisions: snapshot tests skipped (builder-level assertion tests already lock data transforms with more precision than JSON diffs would; user's `feedback_skip_screenshots.md`), `Money` precondition-trap test skipped (XCTest can't catch preconditions on macOS and the trap is unreachable from validated user-input paths via `AddTransactionForm` — already documented on `Money` in M10), manual verification checklist skipped per scope call. 162 tests green (148 → 162, net +14 = 15 new − 1 replaced) |
...
| All-time Gain | **Total return** = unrealized P&L (open positions) + realized P&L (past sells) + income (dividends + staking), resolved 2026-05-29, post-MVP. Was unrealized-only (`gainAllTime` over current holdings), which made closed trades and income invisible. `HoldingsReducer.allTimeBreakdown(...)` walks the ledger once for realized/dividend/staking/totalInvested; `PortfolioMetrics.allTimeGain(...)` sums them with unrealized. The `%` badge = total gain ÷ total invested ever (gross `.buy` cost, excludes staking). Staking is income *and* booked into basis — no double-count because the basis is offset by market value at receipt. Income = dividends **+** staking (matches the app's existing "Income YTD" definition); switch to dividends-only by dropping `stakingIncome` from `realizedPlusIncome` if desired. |
| Performance metric | **Time-weighted return (TWR)** for the Overview YTD card *and* the Annual Performance table (resolved 2026-05-29, post-MVP). Was raw value-change `(end−start)/start`, which conflated price gains with capital added: buying more shares inflated "performance" even with no appreciation. `PortfolioMetrics.timeWeightedReturn(_:)` chains sub-period factors `(Vᵢ − netFlowᵢ)/Vᵢ₋₁`; `HistoryPoint` now carries per-period `netFlow` (buys +, sells −, FX-converted at txn date; stake/dividend/deposit/withdraw are *not* flows). Telescopes to old value-change when flows are zero, so contribution-free histories read identically and all prior tests stayed green. The chart line itself still shows raw total value (users want to see it climb). |
...
- ~~`Folio/Sources/DesignSystem/Formatters.swift` — `FolioFormat` is dead in production (only `DesignSystemPreview` consumes it). Still dead after M5 (the Stocks screen formats via `Money.formatted()` + inline number/percent helpers in row views). Re-tag for M10 or inline into the preview.~~ **Done 2026-05-28** (M10) — inlined as `private enum PreviewFormat` in `DesignSystemPreview.swift`; `Formatters.swift` deleted.
...
- ~~`MockQuoteProvider.swift:48–51,75` — `Decimal(string: "0.92")!` force-unwraps. Programmer-controlled literals, so won't crash, but worth tidying.~~ **Done 2026-05-28** (M10) — replaced with `Decimal(92) / Decimal(100)`-style literals.
...
- Empirically verify the `Holding.ID` stability fix once the Stocks table lands — `ForEach(\.id)` rows should keep selection/hover across `@Query` updates. **Verify next session** by hovering rows during a ⌘R refresh — hover state should not flicker. (Migrated to M11 follow-ups, deferred there.)

**Tackle during M10 (settings + polish):**
- ~~`MacShell.swift:49` — Add-holding bound to `⌘N`, conflicts with the macOS standard "New Window". Pick `⌘+` or `⌘⇧N`.~~ **Done 2026-05-28** (M10) — rebound to `⌘+`.
- ~~`QuoteRefreshCoordinator.swift:135–142` — `Timer.scheduledTimer` keeps firing when the app is backgrounded or the window is closed. Observe `NSApplication.didBecome/ResignActive` to pause/resume.~~ **Done 2026-05-28** (M10) — observers installed in `installLifecycleObservers()`; tokens parked in a `NotificationObserverBag` heap class so deinit-cleanup doesn't need main-actor isolation. Become-active also triggers a catch-up `refreshAll()` if last refresh > interval ago.
- ~~`SidebarFooter.swift:44` — FX closure hard-codes `from == to ? 1 : nil`. Wire a real FX closure that consults `FXRate` rows once the base-currency picker ships.~~ **Stale** — `SidebarFooter` lost its FX-consuming code post-M4 (refresh-status-only). The real wiring happened on the screens themselves: `FXLookup.fxAt(rates:)` now backs `StocksScreen`, `CryptoScreen`, and `OverviewScreen`'s spot reducer (M10, 2026-05-28).
- ~~`QuoteRefreshCoordinator.swift:121–128` — save-error catch swallows *any* error (disk full, schema migration) and still flips status to `.ok`. Filter to constraint-violation codes only; let the rest fail loudly.~~ **Done 2026-05-28** (M10) — `isRecoverablePersistenceError(_:)` classifies `NSCocoaErrorDomain` constraint codes as recoverable (stay `.ok`); anything else flips to `.failed`.
- ~~Replace `print(…)` in `HoldingsReducer.swift:81,100,107` and `QuoteRefreshCoordinator.swift:127` with `os_log`.~~ **Done 2026-05-28** (M10) — new `FolioLog` namespace (subsystems: holdings/quotes/history/persist) replaces all 6 `print` callsites (HoldingsReducer × 3, QuoteRefreshCoordinator, AddTransactionSheet, HistoricalQuoteService).
- ~~Document `Money` mixed-currency `precondition` trap behavior in the type's doc-comment so callers know it's a programmer-error trap.~~ **Done 2026-05-28** (M10) — type-level doc-comment expanded; notes that user-input paths flow through validated `AddTransactionForm` so the trap only fires on programmer error.

**Tackle during M11 (test + verify):**
- ~~Add a `Money` precondition-trap test (or extract a public throwing variant for user-input paths and test that).~~ **Resolved 2026-05-28** (M11) — doc-only. XCTest cannot catch `precondition` on macOS; the trap is structurally unreachable from validated user-input paths (`AddTransactionForm`) and the M10 doc-comment already calls it out as a programmer-error guard. Extracting a `throws` variant adds public surface for a defense that's not needed.
- ~~Add an FX-pair de-duplication test in `QuoteRefreshCoordinatorTests` — `collectFXPairs` distinct-pair behavior is currently untested.~~ **Done 2026-05-28** (M11) — `testCollectFXPairsDedupesAndExcludesBaseCurrency` inserts duplicate EUR txns, asserts the returned pair set is `{(EUR, USD)}` and never contains USD→USD. Required visibility bump on `collectFXPairs` + `FXPair` from `private` → `internal`.
- ~~Replace `MockQuoteProviderTests.testFXRateUSDEURRoundtripCloseToOne` — it asserts a property of hardcoded constants, not behavior.~~ **Done 2026-05-28** (M11) — replaced with `testFXProviderRoundtripConsistency` which loops USD/EUR + USD/GBP through the provider's public API and checks `A→B→A ≈ 1` within `0.01`. Now flags any constant drift instead of restating the literal table.
- ~~Synthetic seed dates (`2024-05-20` syntheticBuyDate vs `2026-05-x` mock txns) rely on sort order — brittle but currently correct. Note at M11.~~ **Done 2026-05-28** (M11) — `SeedData.syntheticBuyDate` now carries a doc-comment stating it must precede every entry in `seedMockTransactions` chronologically and explaining the silent-drift consequence if violated.
- **Deferred from M11**: empirical `Holding.ID` stability verification (M4.5 carry-over). Needs UI-host scaffolding for an automated test; manual hover-during-⌘R check still available next session if desired.
